...
namespace Barbershop.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        [HttpPost("login")]
        public async Task<IActionResult> LogIn([FromBody] LoginRequestModel Credentials)
        {
            if (ModelState.IsValid)
            {
                var result = await _signInManager.PasswordSignInAsync(Credentials.Name, Credentials.Password, false, false);
                if (result.Succeeded)
                {
        ...
        public async Task<IActionResult> Register([FromBody] RegisterRequestModel credentials)
        {
            if (ModelState.IsValid)
            {
                var user = new User { UserName = credentials.Name, Email = credentials.Email };
